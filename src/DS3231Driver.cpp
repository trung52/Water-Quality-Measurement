...
ERROR_CODE DS3231_init(RTC_DS3231& _realTime, TwoWire &_wire)
{
	if (_realTime.begin(&_wire))			// khoi dong module RTC
	{
		...
		connectionStatus_st.ds3231Status = status_et::CONNECTED;
		//_realTime.adjust(DateTime(F(__DATE__), F(__TIME__)));
		return ERROR_NONE;

	} else {
		log_e("RTC module initialized failed!");		
		connectionStatus_st.ds3231Status = status_et::DISCONNECTED;
...
{
	if (connectionStatus_st.ds3231Status ==  status_et::CONNECTED)		// Check RTC DS3231's status
	{
