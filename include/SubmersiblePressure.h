...
#define DENSITY_PURE_WATER 1  // Pure water density normalized to 1 Kg/L or (Kg/dm3)
 ...
 * @return ERROR_CODE
*/
ERROR_CODE submersiblePressure_getDepth(uint32_t _voltage_mv, float &_pressCurrent, float &_depth, float _densityWater){
    _pressCurrent = (float)_voltage_mv / 120; //Sense Resistor:120ohm
    _depth = (_pressCurrent - CURRENT_INIT) * (DEPTH_RANGE/ _densityWater/ 16.0); // Calculate depth from current readings
    if(_depth < 0){
        _depth = 0.0;
    }
    return ERROR_NONE;
